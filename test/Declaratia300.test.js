...
describe("Declaratia300", function () {
  let declaratia300;
  let owner;
  let firma1;
  let firma2;
  ...
  describe("Deployment", function () {
    ...
    it("Should have correct initial state", async function () {
      ...
      expect(await declaratia300.firmaAmount(firma2.address)).to.equal(0);
    });
  });

  describe("payForDeclaratia300", function () {
    ...
    it("Should accumulate payments for the same firm", async function () {
      ...
      await declaratia300.payForDeclaratia300(firma1.address, amount1);
      await declaratia300.payForDeclaratia300(firma1.address, amount2);
      
      expect(await declaratia300.firmaAmount(firma1.address)).to.equal(amount1 + amount2);
    });
    ...
    it("Should handle multiple firms independently", async function () {
      ...
      await declaratia300.payForDeclaratia300(firma1.address, amount1);
      await declaratia300.payForDeclaratia300(firma2.address, amount2);
      
      expect(await declaratia300.firmaAmount(firma1.address)).to.equal(amount1);
      expect(await declaratia300.firmaAmount(firma2.address)).to.equal(amount2);
    });
    ...
    it("Should handle large amounts", async function () {
      ...
      await declaratia300.payForDeclaratia300(firma1.address, largeAmount);
      
      expect(await declaratia300.firmaAmount(firma1.address)).to.equal(largeAmount);
    });
    ...
    it("Should allow anyone to call the function", async function () {
      ...
      await declaratia300.connect(firma1).payForDeclaratia300(firma2.address, amount);
    ...
    it("Should handle multiple payments in sequence", async function () {
      ...
      for (let i = 0; i < amounts.length; i++) {
        await declaratia300.payForDeclaratia300(firma1.address, amounts[i]);
        total += amounts[i];
        expect(await declaratia300.firmaAmount(firma1.address)).to.equal(total);
      }
    });
  });
  ...
  describe("firmaAmount mapping", function () {
    it("Should return zero for unregistered firms", async function () {
      expect(await declaratia300.firmaAmount(firma3.address)).to.equal(0);
    });
  ...
  describe("Edge cases", function () {
    it("Should handle maximum uint256 values", async function () {
      ...
      await declaratia300.payForDeclaratia300(firma1.address, maxAmount);
      
      expect(await declaratia300.firmaAmount(firma1.address)).to.equal(maxAmount);
    });

    it("Should handle address(0)", async function () {
      const amount = ethers.parseEther("100");
      
      await declaratia300.payForDeclaratia300(ethers.ZeroAddress, amount);
      
      expect(await declaratia300.firmaAmount(ethers.ZeroAddress)).to.equal(amount);
    });
  });
  ...
  describe("Gas optimization", function () {
    it("Should use reasonable gas for payments", async function () {
      ...
      const tx = await declaratia300.payForDeclaratia300(firma1.address, amount);
