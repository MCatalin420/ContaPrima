...
describe("Declartia100", function () {
  let declartia100;
  let owner;
  let firma1;
  let firma2;
  ...
  describe("Deployment", function () {
    ...
    it("Should have correct initial state", async function () {
      ...
      expect(await declartia100.firmaAmount(firma2.address)).to.equal(0);
    });
  });

  describe("payForDeclartia100", function () {
    ...
    it("Should accumulate payments for the same firm", async function () {
      ...
      await declartia100.payForDeclartia100(firma1.address, amount1);
      await declartia100.payForDeclartia100(firma1.address, amount2);
      
      expect(await declartia100.firmaAmount(firma1.address)).to.equal(amount1 + amount2);
    });
    ...
    it("Should handle multiple firms independently", async function () {
      ...
      await declartia100.payForDeclartia100(firma1.address, amount1);
      await declartia100.payForDeclartia100(firma2.address, amount2);
      
      expect(await declartia100.firmaAmount(firma1.address)).to.equal(amount1);
      expect(await declartia100.firmaAmount(firma2.address)).to.equal(amount2);
    });

    it("Should handle zero amount", async function () {
      await declartia100.payForDeclartia100(firma1.address, 0);
    ...
    it("Should handle large amounts", async function () {
      ...
      await declartia100.payForDeclartia100(firma1.address, largeAmount);
      
      expect(await declartia100.firmaAmount(firma1.address)).to.equal(largeAmount);
    });
    ...
    it("Should allow anyone to call the function", async function () {
      ...
      await declartia100.connect(firma1).payForDeclartia100(firma2.address, amount);
    ...
    it("Should handle multiple payments in sequence", async function () {
      ...
      for (let i = 0; i < amounts.length; i++) {
        await declartia100.payForDeclartia100(firma1.address, amounts[i]);
        total += amounts[i];
        expect(await declartia100.firmaAmount(firma1.address)).to.equal(total);
      }
    });
  });
  ...
  describe("firmaAmount mapping", function () {
    it("Should return zero for unregistered firms", async function () {
      expect(await declartia100.firmaAmount(firma3.address)).to.equal(0);
    });
  ...
  describe("Edge cases", function () {
    it("Should handle maximum uint256 values", async function () {
      ...
      await declartia100.payForDeclartia100(firma1.address, maxAmount);
      
      expect(await declartia100.firmaAmount(firma1.address)).to.equal(maxAmount);
    });

    it("Should handle address(0)", async function () {
      const amount = ethers.parseEther("100");
      
      await declartia100.payForDeclartia100(ethers.ZeroAddress, amount);
      
      expect(await declartia100.firmaAmount(ethers.ZeroAddress)).to.equal(amount);
    });
  });
  ...
  describe("Gas optimization", function () {
    it("Should use reasonable gas for payments", async function () {
      ...
      const tx = await declartia100.payForDeclartia100(firma1.address, amount);
